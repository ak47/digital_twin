...
- **Conversation persistence (optional)**: When **`DATABASE_URL`** points at **Cloud SQL PostgreSQL** (Terraform **`cloud_sql.tf`**), messages are stored per-row in **`conversations`** / **`messages`** via SQLAlchemy models in **`db/models.py`** and **`conversation_store.py`**. Schema changes use **Alembic** (`alembic/versions/`); **Deploy API** runs migrations before each Cloud Run revision when conversation DB variables are set. Public chat still uses **`X-Session-Id`** as the conversation UUID. **`GET /api/conversations/{id}?after=`** supports visitor polling for owner replies.
- **Admin (optional)**: **`/admin/*`** routes use **Google OAuth** + email allowlist (**`ADMIN_ALLOWED_EMAILS`**), signed **`dt_admin`** cookie (2-hour TTL), and credentialed CORS. Owner inbox, replies, instructions, and archive are API-only in this repo; UI lives in **`ak47.github.io/no_ego`**.
- **Escalation email (optional)**: When the twin reply includes **`<<ESCALATE>>`**, the API flags **`needs_attention`** and sends a debounced Gmail alert via **`escalation_email.py`** (reuses digest Gmail credentials).
